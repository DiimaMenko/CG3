...
void Segment::setStart(Point *startP){
    this->startPoint = startP;
}

void Segment::setEnd(Point *endP){
    this->endPoint = endP;
}
...
double Segment::Angle(Segment *seg)
{
    if((this->B * seg->getA() - seg->getB() * this->A) < 0)
        return	PI - acos((this->B * seg->getB() + this->A * seg->getA()) /
                     (sqrt(this->A * this->A + this->B * this->B) *
                     sqrt(seg->getA() * seg->getA() + seg->getB() * seg->getB())));
    else
